    ...
    switch (page){
        ...
        return(
             <div style={{height:'100%'}}>
                <div onTouchStart={e=>setInstWidth(0)} style={{height:'100%',backgroundColor:'#282c34',opacity:instWidth,width:`${instWidth}%`,transition:'.5s',position:'absolute',display:'flex',justifyContent:'center',alignItems:'center',flexDirection:'column',zIndex:'999',textAlign: 'left'}}>
                    <h1>{text.instructions.head[lang]}</h1>
                    <h2>{text.instructions.body[props.app][lang].name}</h2>
                    <h4 style={{margin:'0 10vmin'}}>{text.instructions.body[props.app][lang].text}</h4>
                </div>
